/**
 * Taqeem Lightweight QR Code Generator
 * Zero-dependency, offline-first client-side QR generation for Judge Pass & Audience Voting
 * MIT License
 */
(function (root, factory) {
  ...
  QR8bitByte.prototype = {
    ...
    write: function (buffer) {
      for (let i = 0; i < this.data.length; i++) {
        ...
        if (c > 127) {
          const utf8 = unescape(encodeURIComponent(this.data[i]));
          for (let u = 0; u < utf8.length; u++) buffer.put(utf8.charCodeAt(u), 8);
        } else {
          buffer.put(c, 8);
        }
      }
  ...
  return {
    renderToCanvas: function (canvas, text, options) {
      ...
      const cellSize = (size - margin * 2 * 4) / moduleCount;
